...
MODEL = "gemini-3.5-flash"  # current free-tier model
_client = None


def get_client() -> genai.Client:
    """Lazy client creation so this module is safe to import from Streamlit
    (which sets GEMINI_API_KEY itself and must not be blocked by getpass)."""
    global _client
    if _client is None:
...
def call_llm(system_prompt: str, user_prompt: str, max_tokens: int = 3000) -> str:
    """Thin wrapper around the Gemini API used by every chain in this notebook.

    thinking_budget=0 disables extended "thinking" tokens: our tasks are
    straightforward JSON extraction, and thinking tokens otherwise eat into
    max_output_tokens and can truncate the JSON response before it completes."""
