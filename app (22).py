...
# ---------------------------------------------------------------------------
# Self-hosted LLM server address (Mistral/Llama running on Kaggle/Colab,
# exposed via ngrok). No third-party AI API or key involved - these just
# point at a server you control.
# ---------------------------------------------------------------------------
if "LLM_SERVER_URL" in st.secrets:
    os.environ["LLM_SERVER_URL"] = st.secrets["LLM_SERVER_URL"]
if "LLM_API_KEY" in st.secrets:
    os.environ["LLM_API_KEY"] = st.secrets["LLM_API_KEY"]

if not os.environ.get("LLM_SERVER_URL"):
    url = st.sidebar.text_input(
        "Model server URL (ngrok link)",
...
if not os.environ.get("LLM_API_KEY"):
    server_key = st.sidebar.text_input("Model server key", type="password")
    if server_key:
        os.environ["LLM_API_KEY"] = server_key

import pipeline as pl  # noqa: E402  (import after server URL is set)
...
if not os.environ.get("LLM_SERVER_URL"):
    st.warning("Paste your model server's ngrok URL in the sidebar to start "
               "(run the model-server notebook on Kaggle/Colab first).")
    st.stop()
